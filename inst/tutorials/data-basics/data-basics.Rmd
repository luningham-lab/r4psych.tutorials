...
In this tutorial, you will learn how to use R to inspect the contents of a data frame or tibble. Data frames and tibbles are R's structures for storing tabular data. "Tabular data" refers to data organized into rows and columns. If you are given a tabular dataset in R, it will almost certainly come in one of these structures.
...
This tutorial was originally written by authors of the `learnr` package:

Aden-Buie G, Schloerke B, Allaire J, Rossell Hayes A (2025). _learnr: Interactive
  Tutorials for R_.
  
This tutorial was edited and adapted by Justin Luningham for teaching purposes only.

The readings in this tutorial follow [_R for Data Science_](http://r4ds.had.co.nz/), sections 1.2.1 and 3.1.2.
...
Also note that `ggplot2` is loaded as part of the `tidyverse`.

After that, you will be able to access any object in ggplot2—including `mpg`—until you close R.

###

Did you notice how much information was inside `mpg`? Sometimes the contents of a data frame do not fit on a single screen, which makes them difficult to inspect. We'll look at an alternative to using and examining data frames soon. But first let's get some help...
...
Notice that objects created by you or your colleagues simply within your `R` environment will not have a help page (unless you make one). For example, in our prior tutorial, we created the object `thirteen <- 8 + 5`. If we then ran `?thirteen`, we'd receive the message

```
No documentation for ‘thirteen’ in specified packages and libraries:
you could try ‘??thirteen’
```

### Exercises

Use the code chunk below to load help pages and answer the following questions.
...
The `flights` data frame in the nycflights13 package is an example of a _tibble_. Tibbles are data frames with some extra properties.
...
You've met R's basic table structures—data frames and tibbles; and you have learned how to inspect their contents. You have also learned about help pages, which are extremely useful for viewing the details of functions and built-in datasets.
