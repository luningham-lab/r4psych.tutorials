...
We can call on `emmeans()` to get the marginal means, averaged over other variables. This is usually only helpful if we have a categorical predictor or we have interaction terms in the model. 
...
```{r zip9, exercise = TRUE, exercise.setup = "zip8"}
emmeans(zip.fit, ~ group, mode = "count") # large counts because zeros excluded
2.76/6.83 # estimated rate ratio!
```

```{r zip10, exercise = TRUE, exercise.setup = "zip8"}
emmeans(zip.fit, ~ group, mode = "zero")
...
```{r zip11, exercise = TRUE, exercise.setup = "zip8"}
...
We will examine some of the functionality of `tab_model` in our final lessons.
